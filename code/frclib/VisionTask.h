...
public:
    ...
    {
        TLevel(INIT);
        TEnterMsg(("camera=%p,type=%d,colorTh=%p,sizeTh=%d,criteria=%p,numCrit=%d,waitPeriod=%4.2f",
                    camera, imageType, colorThresholds, sizeThreshold,
                    filterCriteria, numCriteria, taskWaitPeriod));
