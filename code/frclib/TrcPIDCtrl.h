...
public:
     ...
     * @param initialOutput Specifies the initial prevOutput value,
    ...
    TrcPIDCtrl(
        ...
        double Kd,
        float  tolerance = 0.0,
        UINT32 settlingTime = 0,
        UINT32 pidCtrlOptions = 0,
    ...
    {
        TLevel(INIT);
        TEnterMsg(("ID=%s,Kp=%f,Ki=%f,Kd=%f,tolerance=%f,settlingTime=%d,"
                   "options=%x,initialOuptut=%f",
                   idString, Kp, Ki, Kd, tolerance, settlingTime,
                   pidCtrlOptions, initialOutput));
        ...
        )
    {
        ...
        if (m_pidCtrlOptions & PIDCTRLO_SPEED_CTRL)
        {
            output += m_prevOutput;
        }
