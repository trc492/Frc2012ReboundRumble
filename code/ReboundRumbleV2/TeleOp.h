    ...
    )
{
    ...
    {
        float speed = m_shooterJoystick.GetYWithDeadband();
        m_pickup.SetConveyorSpeed(speed);
    }
    ...
    else if (port == JSPORT_SHOOTER)
    {
        ...
        case Logitech_Btn8:
            ...
            if (fPressed)
            {
                ...
                if (m_hellModeEnabled)
                {
                    m_whiteLight.Set(0.5, 0.5);
                    m_shooter.StopPID();
                    m_shooter.SetContinuousMode(false);
                }
                else
                {
                    m_whiteLight.Set(false);
                    m_shooter.SetContinuousMode(true);
                }
            }
            break;
...
void TrcRobot::NotifyEIODin(UINT32 channel, UINT32 state)
{
    switch (channel)
    {
        case DSSW_CONVEYOR_CTRL_ENABLED:
            m_conveyorControlEnabled = (state == 1);
            break;
    }
}   //NotifyEIODin
#endif
